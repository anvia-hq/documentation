...
Use Anvia request fields for messages, documents, tools, token limits, temperature, and cancellation. Put provider-specific options in completion `additionalParams`. Validate them against the selected Anthropic SDK and model; the adapter does not make vendor options portable.
...
The package is ESM, includes declarations, and peers on `@anvia/core >=0.7.1 <1.0.0`. It depends on the official Anthropic and Anthropic Vertex SDKs. Keep both clients server-side.
