...
The adapter's default local client uses insecure connections and is not appropriate for a protected remote service. `WeaviateVectorClient.close()` closes the adapter-created client, and `nativeClient()` exposes it; lifecycle-managed applications can instead inject their own client and close it themselves.
