...
The image factory selects separate provider implementations through its `api` discriminant. Both return decoded bytes and reject responses without valid image content.
...
There is no Anvia speech-generation model in this package. Live sessions, files, caches, batches, tuning, and other native Google GenAI modules are not exposed as Anvia factories. Use the injected `GoogleGenAI` client directly when native APIs are needed.
