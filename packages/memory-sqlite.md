...
The package is ESM-only and should use a version compatible with its declared `@anvia/core` dependency range. It uses `node:sqlite`, so run it on a Node.js version that provides `DatabaseSync`.
