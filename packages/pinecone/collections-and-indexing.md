...
With Pinecone SDK 9, `spec` must describe a supported serverless or BYOC index. Legacy pod
creation, metadata-indexing schemas, `sourceCollection`, and `sourceBackupId` are not supported for
new indexes. Provision those resources outside the adapter when migrating an older deployment.
