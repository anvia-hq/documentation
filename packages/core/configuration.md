...
Configure stable behavior on `AgentBuilder`, then add run-specific behavior on the `PromptRequest` it creates.
...
const agent = new AgentBuilder('support', model)
  ...
  .context('Refunds are available for 30 days.', 'refund-policy')
...
Builder configuration applies to every prompt from that agent. IDs should be stable because stores, traces, Studio, and agent-as-tool relationships use them.
