...
| Need | Primitive |
| --- | --- |
| One model call returns one validated object | [`generateCompletion(...)`](/sdk/structured-output/parsed-completion) |
| An agent uses tools, memory, or retrieval first | [Agent output schema](/sdk/structured-output/agent-output) |
| Existing content must be converted into fields | [`extract()`](/sdk/structured-output/extractors) |
| A tool returns typed data to the model | Tool `outputSchema` |
...
const result = await generateCompletion({
    prompt: 'Checkout is unavailable for all customers.',
    model,
