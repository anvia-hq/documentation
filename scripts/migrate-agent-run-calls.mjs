...
function migratedArguments(call) {
  ...
  if (call.expression.name.text !== 'generate' && call.expression.name.text !== 'stream') return
  ...
  return [
    ts.factory.createObjectLiteralExpression(
      [
        ts.factory.createPropertyAssignment(inputProperty(input), input),
        ...(options?.properties ?? []),
      ],
      true,
    ),
...
console.log(`Migrated ${migratedCalls} Agent run calls to object-only input.`)
