...
function migratedArguments(call) {
  ...
  if (call.expression.text === 'generateImage' || call.expression.text === 'generateSpeech') {
    const inputName = call.expression.text === 'generateImage' ? 'prompt' : 'text'
    return [
      ts.factory.createObjectLiteralExpression(
        [ts.factory.createPropertyAssignment(inputName, input), ...options.properties],
        true,
      ),
    ]
  }

  if (call.expression.text !== 'transcribe') return
  const audioProperties = [ts.factory.createPropertyAssignment('data', input)]
  const remainingProperties = []
  for (const property of options.properties) {
    if (propertyName(property) === 'filename' || propertyName(property) === 'mediaType') {
      audioProperties.push(property)
    } else {
    ...
    ts.factory.createObjectLiteralExpression(
      [
        ts.factory.createPropertyAssignment(
          'audio',
          ts.factory.createObjectLiteralExpression(audioProperties, true),
        ),
        ...remainingProperties,
      ],
      true,
...
console.log(`Migrated ${migratedCalls} media calls to object-only options.`)
